"""
Standalone deployment inference script for the AFIB / AFLT DINO model.

Pipeline: raw single-lead (II) ECG signal
          -> preprocess (denoise + resample, RR/HR/PSD features, DINO image)
          -> ECGDinoModel (+ PEFT/LoRA adapter "AFIB,AFLT,Others")
          -> alpha-scaled sigmoid + per-class threshold
          -> {AFIB, AFLT, Others} probs & binary predictions
...
Usage:
    python infer_dino_afib_aflt.py --checkpoint /path/to/adapter/AFIB,AFLT,Others \
...
NUM_FEATURES = 6
LABELS = ["AFIB", "AFLT", "Others"]
NUM_LABELS = len(LABELS)

BEST_ALPHAS = np.array([1.35, 0.6, 1.45])
BEST_THRESHOLDS = np.array([0.17, 0.74, 0.67])
...
# --------------------------------------------------------------------------
def apply_alpha_threshold_multilabel(logits, alphas, thresholds):
    """
    logits: (N, C)
    alphas: (C,)
    thresholds: (C,)
    """
    probs = expit(logits * alphas)
...
# --------------------------------------------------------------------------
def load_afib_aflt(checkpoint_path, device=None):
    """
    Builds everything infer_afib_aflt() needs (image processor, denoiser,
    model + LoRA adapter) exactly once. Pass the returned dict into
    infer_afib_aflt() as `resources` for every subsequent call instead of a
    checkpoint path -- this is what avoids re-loading the model from disk
    on every window.
    """
...
# Inference
# --------------------------------------------------------------------------
def infer_afib_aflt(signal, fs, unit, checkpoint_path=None, device=None, pipeline=None,
                     model=None, resources=None):
    """
    Run the AFIB/AFLT DINO model end-to-end on a single raw signal.

    Fast path (recommended for repeated / windowed calls):
        resources = load_afib_aflt(checkpoint_path)   # once
        infer_afib_aflt(window, fs, unit, resources=resources)   # per window

    Slow path (kept for backwards compatibility / one-off calls):
        infer_afib_aflt(signal, fs, unit, checkpoint_path)
    ...
    Returns a dict:
        {
            "AFIB":   {"prob": float, "pred": int},
            "AFLT":   {"prob": float, "pred": int},
            "Others": {"prob": float, "pred": int},
        }
    """
    if resources is not None:
        ...
        if model is None:
            if checkpoint_path is None:
                raise ValueError(
                    "infer_afib_aflt needs either `resources=load_afib_aflt(...)`, "
                    "a pre-loaded `model`, or a `checkpoint_path` to load one from."
                )
            model = load_model(checkpoint_path, device)
    ...
    preds, probs = apply_alpha_threshold_multilabel(
        logits, BEST_ALPHAS, BEST_THRESHOLDS
    )

    result = {
        LABELS[i]: {"prob": float(probs[0][i]), "pred": int(preds[0][i])}
...
def main():
    parser = argparse.ArgumentParser(description="AFIB/AFLT DINO inference")
    parser.add_argument(
        "--checkpoint",
        required=True,
        help="Path to the PEFT/LoRA adapter dir for the AFIB,AFLT,Others head",
    )
    # parser.add_argument(
    ...
    result = infer_afib_aflt(signal, args.fs, args.unit, args.checkpoint)
