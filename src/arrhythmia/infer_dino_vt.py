...
Usage:
    python infer_dino_vt.py --checkpoint /path/to/adapter/VT \
...
NUM_FEATURES = 6
NUM_LABELS = 2  # binary: [negative, VT]

BEST_ALPHA = 1.0500000000000005
BEST_THRESHOLD = 0.01
...
# --------------------------------------------------------------------------
def load_vt(checkpoint_path, device=None):
    """
    Builds everything infer_vt() needs (image processor, denoiser, model +
    LoRA adapter) exactly once. Pass the returned dict into infer_vt() as
...
# Inference
# --------------------------------------------------------------------------
def infer_vt(signal, fs, unit, checkpoint_path=None, device=None, pipeline=None,
             model=None, resources=None):
    """
    Run the VT DINO model end-to-end on a single raw signal.

    Fast path (recommended for repeated / windowed calls):
        resources = load_vt(checkpoint_path)   # once
        infer_vt(window, fs, unit, resources=resources)   # per window

    Slow path (kept for backwards compatibility / one-off calls):
        infer_vt(signal, fs, unit, checkpoint_path)
        -- rebuilds the image processor + reloads the model from disk
           on every single call. Fine for a single ad-hoc inference,
           wasteful in a loop.

    Returns a dict:
        {"VT": {"prob": float, "pred": int}}
    """
    if resources is not None:
        ...
        if model is None:
            if checkpoint_path is None:
                raise ValueError(
                    "infer_vt needs either `resources=load_vt(...)`, a pre-loaded "
    ...
    return {"VT": {"prob": float(probs[0]), "pred": int(preds[0])}}


def main():
    parser = argparse.ArgumentParser(description="VT DINO inference")
    parser.add_argument(
        "--checkpoint", required=True, help="Path to the PEFT/LoRA adapter dir for the VT head"
    )
    # parser.add_argument(
    ...
    result = infer_vt(signal, args.fs, args.unit, args.checkpoint)
